...
- [x] z roll：根據人眼斜率旋轉 => 事實是不應該轉
- [x] 旋轉球體動畫：可能是因為目前沒有寫好著色器之類的，看不出旋轉

### 推定使用者頭部 3D 座標
- [-] 人臉太陽穴近大遠小 + webcam 圖片二維座標
  - faceApi 下用的是使用者頭部 bounding box
- [x] 由此在 sensitivity 範圍內調整相機座標
...
- [-] 檢視模式變換：
  ...
  - [x] 模式 2：環繞目標模型（視覺收斂）
  - [x] TODO：改成 on/off 而非現在的 slider

### xz 網格與 xy 網格
- [x] 有益於營造空間感 + debug

### 顯示準心
- [x] 目前會把準心印在 xy 網格上，是「無限遠處的視覺收斂點」到「相機」之間連線，在該面上的交點
- [x] 還會把這條線投影在 xy & xz 網格上
- [x] 以上東西有益於 debug
...
### 效能與 fog
- [x] 允許使用者調整渲染範圍 & fog 範圍

### Load File
- [x] 允許使用者上傳 3D 模型檔案（例如 `.obj`, `.gltf` 等），並在場景中顯示。
- 支援拖曳上傳或點擊選擇檔案 (Guest mode)。
- 顯示公開模型（從 gallery 來的使用者；依照 id 讀取 model）

### `class Panel`
- [x] 用於記錄與提供 UI 調整以下設定，並把更改後的設定存 localStorage（不存 SQL 因為一個使用者可能有多裝置）
...
- [x] w a s d arrowUp arrowDown
    這是常見的前後左右移動 + 抬頭低頭
- ~~space to hold the screen（我想做這個）~~ **不合物理邏輯，刪除**
    按住空白鍵以固定相機位置，鬆手以重新使用人頭座標。相當於 cam pos += 按下後人頭位置 - 按下前人頭位置
- ~~[ ] 滑鼠拖拽以移動？~~ **可能會跟「點一下螢幕以使得焦點脫離 panel」衝突，放棄**
