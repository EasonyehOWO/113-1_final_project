--- 建議使用setup_database.php來初始化資料庫 ---

---- 冪等腳本式初始化（舊資料會被 drop table 掉）
---- 可以透過 `sudo mysql < DB_structure.sql` 來匯入
-- 建立資料庫
CREATE DATABASE IF NOT EXISTS `CVML` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `CVML`;

CREATE USER IF NOT EXISTS 'CVML'@'localhost' IDENTIFIED BY '114DWP2025';
GRANT ALL PRIVILEGES ON `CVML`.* TO 'CVML'@'localhost';
FLUSH PRIVILEGES;
