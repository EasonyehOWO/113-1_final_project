...
## 5. 視覺收斂模式 (Visual Convergence / Hologram Mode)

這是專案後期的重大功能更新，與上述的標準視窗模式邏輯截然不同。

### 模式比較
1.  **視窗模式 (Off-Axis View)**:
    - 預設模式。
    - 相機方向：永遠垂直於螢幕 (`rotation = (0,0,0)`)。
    - 投影矩陣：**非對稱 (Off-axis)**。
    - 效果：螢幕像是一扇窗戶，你可以探頭看窗戶後面的東西。

2.  **視覺收斂模式 (Hologram Mode)**:
    - 選項：`視覺收斂模式 (LookAt Center)`
    - 相機方向：永遠注視場景中心 (`lookAt(0,0,0)`)。
    - 投影矩陣：**標準對稱 (Symmetric)**。
    - 效果：物體像是浮在空中的「全息投影」。當你向左移動時，你會看到物體的左側面（因為相機繞著物體轉）。

### 技術實作細節
此模式下，我們停用了 `Off-Axis Projection` 的 Shift 計算，改為標準的 `PerspectiveProjection`。然而，為了支援 **真實透視 (Physics Mode)**，我們引入了動態 FOV 機制：

- **當 Physics Mode 開啟時**：
  由於物體在收斂模式下是視覺焦點，若純粹靠近而不改變 FOV，物體可能迅速變大佔滿畫面。為了維持「透過固定視窗看全息物件」的感覺，我們動態計算 FOV：
  $$ FOV_{new} = 2 \times \arctan \left( \tan(\frac{FOV_{base}}{2}) \times \frac{Z_{ref}}{Z_{current}} \right) $$
  這會產生廣角效果，讓物體在視覺上保持適當的大小與距離感，同時擴大周邊視野。

---

## 6. 校準與互動功能 (Calibration & Interaction)

為了讓上述的物理模型能與真實世界接軌，本系統提供了以下校準工具：

### 6.1 PPI 校準 (Pixel Per Inch)
*   **功能**：確保螢幕上的「1單位 (10cm)」真的等於實體世界的 10 公分。
*   **操作**：
    1.  點擊控制面板中的「Pixels Per Inch (PPI)」滑桿。
    2.  按住 **空白鍵 (Space)**：螢幕會顯示紅色的校準尺。
    3.  拿一把真實的尺貼在螢幕上。
    4.  按鍵盤左右鍵微調 PPI 數值，直到螢幕上的 5cm 刻度與您的直尺吻合。

### 6.2 近截面調整 (Near Plane Adjustment)
*   **功能**：調整渲染的起始距離，可用來視覺化「立體視窗」的邊界。
*   **操作**：
    *   在控制面板的「Distance & Fog」區塊調整「近截面 (Near)」滑桿。
    *   **較小數值 (0.1)**：允許物體飛出螢幕，直到極近距離（具有強烈的立體突出感）。
    *   **較大數值 (如 5.0)**：若設為物體所在的深度，則物體突出的部分會被「切掉」。這可以用來驗證立體深度是否正確。

### 6.3 放大模式 (Zoom Mode) 的舒適 FOV 計算
*   在非 Physics Mode 下，我們並非單純使用隨機的 FOV。
*   系統會根據您的 PPI，自動計算出在 **60cm 標準視距 (Reference Distance)** 下最舒適的視野。
*   **計算公式**：
    $$ FOV = 2 \times \arctan\left( \frac{\text{螢幕虛擬高度}}{2 \times 6.0} \right) $$
    *(其中 6.0 代表 60cm 的虛擬單位長度)*
*   **優點**：
    *   **大螢幕 (如 27吋)**：計算出的 FOV 較大 (廣角)，讓物體看起來大小適中。
    *   **小螢幕 (如 13吋)**：計算出的 FOV 較小 (望遠)，自動放大物體以補償螢幕較小的問題。
    *   這確保了無論您使用筆電或桌機，茶壺在螢幕上看起來的大小都會差不多，提供一致的視覺體驗。
